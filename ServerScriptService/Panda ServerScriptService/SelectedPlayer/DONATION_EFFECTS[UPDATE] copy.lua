...
-- Crear eventos de donación si no existen
local DonationNotify = userPanelFolder:FindFirstChild("DonationNotify")
if not DonationNotify then
	DonationNotify = Instance.new("RemoteEvent")
	DonationNotify.Name = "DonationNotify"
	DonationNotify.Parent = userPanelFolder
end

local DonationMessage = userPanelFolder:FindFirstChild("DonationMessage")
if not DonationMessage then
	DonationMessage = Instance.new("RemoteEvent")
	DonationMessage.Name = "DonationMessage"
	DonationMessage.Parent = userPanelFolder
end
...
local function onPurchase(player, assetId, wasPurchased, isGamepass)
	...
	-- Notificar clientes (NEW: UserPanel system)
	DonationNotify:FireAllClients(player.UserId, price, creatorId)
...
function handleCommand(player, message)
	...
	if command == ";fd" and args[2] and args[3] then
		...
		-- Simular donación (NEW: UserPanel system)
