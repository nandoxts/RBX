...
function Overflow.updateAvailableIcons(alignment)
	...
	local holderUIList = holder:FindFirstChild("UIListLayout")
	if not holderUIList then
		-- Si no existe UIListLayout, crearlo
		holderUIList = Instance.new("UIListLayout")
		holderUIList.FillDirection = Enum.FillDirection.Horizontal
		holderUIList.HorizontalAlignment = Enum.HorizontalAlignment.Left
		holderUIList.VerticalAlignment = Enum.VerticalAlignment.Center
		holderUIList.Padding = UDim.new(0, 8)
		holderUIList.Parent = holder
	end
...
function Overflow.getRealXPositions(alignment, orderedIcons)
	...
	local holderUIList = holder:FindFirstChild("UIListLayout")
	local topbarInset = (holderUIList and holderUIList.Padding.Offset) or 8
...
function Overflow.updateBoundary(alignment)
	...
	local holderXSize = holder.AbsoluteSize.X
	local topbarInset = (holderUIList and holderUIList.Padding.Offset) or 8
	local topbarPadding = (holderUIList and holderUIList.Padding.Offset) or 8
